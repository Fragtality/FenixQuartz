...
But if handling numeric Values is easier for your Software/Hardware Combination, you can enable the Raw-Value/Offset Mode (**rawValues**) in the Config-File.
...
You can also choose to export the numeric Raw-Values to L-Vars (**useLvars**) (aka Raw-Value/L-Var Mode) - you need to change both. In that Mode, FSUIPC is not required for FenixQuartz to run. FSUIPC is just needed for Offsets, everything else is done via the MobiFlight WASM Module by now.<br/><br/>
